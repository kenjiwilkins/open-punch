...
- SST（Ion / v4系）（Pulumi ベース）でインフラ定義
...
infra/, sst.config.ts             # SST（Ion / v4系）
```
...
5. **多拠点・多国対応（日本＋豪州）**。`occurredAt` は **UTC 保存**。タイムゾーンは**拠点（Location）単位の IANA名**で持ち、`businessDate` は拠点TZで算出する。固定オフセットは使わない（豪州の DST・30分刻みで壊れる）。`computeBusinessDate(occurredAt, timeZone, cutoffHour=0)` に閉じ込める。
6. **打刻種別は `CLOCK_IN` / `CLOCK_OUT` の2種のみ**。休憩は「一旦退勤 → 戻ったら出勤」で表現する（`BREAK_*` は作らない）。
7. **多拠点（Location）対応**。各拠点が IANA タイムゾーンを持つ。Worker は Location に所属し、kiosk は1拠点に紐づく。admin は拠点スコープで見る。
...
13. **業務日は拠点TZで算出**。`computeBusinessDate(occurredAt, timeZone, cutoffHour=0)` の純関数に閉じ込める。既定は暦日（cutoffHour=0）、拠点ごとに締め時刻設定可。`businessDate` と `timeZone` は打刻時に確定保存。
